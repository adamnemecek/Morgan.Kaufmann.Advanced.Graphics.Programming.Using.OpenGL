...
loadStreamlines(char *filename)
{
	...
	for (j = 0; j < numVertices[i]; j++)
	    fscanf(fp, "%f %f %f %f %f %f/n", &x[i][j], &y[i][j], &z[i][j], &tx[i][j], &ty[i][j], &tz[i][j]);
    }
...
XXXX
		...
		glVertex3f(x[i][j] + 0.01*cos(2*M_PI*c/(float)CYL_SIDES),
			   y[i][j] + 0.01*sin(2*M_PI*c/(float)CYL_SIDES),
			   z[i][j]);
XXXX
		glNormal3f(cos(2*M_PI*((c+1)%CYL_SIDES)/(float)CYL_SIDES),
			   sin(2*M_PI*((c+1)%CYL_SIDES)/(float)CYL_SIDES), 0.0);
		glVertex3f(x[i][j] + 0.01*cos(2*M_PI*((c+1)%CYL_SIDES)/(float)CYL_SIDES),
			   y[i][j] + 0.01*sin(2*M_PI*((c+1)%CYL_SIDES)/(float)CYL_SIDES),
			   z[i][j]);
	    }
...
loadTextureMatrix()
{
    ...
    m[0] = 0.5 * -light[0]; m[4] = 0.5 * -light[1]; m[ 8] = 0.5 * -light[2];m[12] = 0.5;
    m[1] = 0.5 * view[0];   m[5] = 0.5 * view[1];   m[ 9] = 0.5 * view[2];  m[13] = 0.5;
